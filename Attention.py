...
"""
- 计算频率序列:
  o torch.arange(0, dim, 2)[ : (dim // 2) ].float() 生成了一个从0开始, 步长为2的序列, 其长度为dim的一半。
  o 每个元素除以dim后取theta的倒数, 得到一个频率序列freqs。这一步是为了生成适合旋转嵌入的频率。

- 生成时间序列:
  o t = torch.arange(end, device=freqs.device) 生成一个从0到end的序列, 长度为end。end通常是序列的最大长度。

- 计算频率的外积
  o freqs = torch.outer(t, freqs).float() 计算时间序列t和频率序列freqs的外积, 得到一个二维矩阵freqs。
  每一行是时间序列t的元素乘以频率序列freqs的元素。

- 计算实部和虚部
  o freqs_cos = torch.cos(freqs) 计算频率矩阵freqs的余弦值, 得到旋转嵌入的实部。
  o freqs_sin = torch.sin(freqs) 计算频率矩阵freqs的正弦值, 得到旋转嵌入的虚部。

最终, 该函数返回两个矩阵freqs_cos和freqs_sin, 分别表示旋转嵌入的实部和虚部, 用于后续的计算。
