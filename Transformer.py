...
class Transformer(PreTrainedModel):
    ...
    def __init__(self, args: ModelConfig = None):
        ...
        # 预计算相对位置嵌入的频率
        freqs_cos, freqs_sin = Attention.precompute_freqs_cis(self.args.dim // self.args.n_heads, self.args.max_seq_len)
        # register_buffer是nn.Module内置的预设方法，专门用于管理 “不需要训练的张量（非参数张量）”。
    ...
    def forward(self, tokens: torch.Tensor, targets: Optional[torch.Tensor] = None, **keyargs) -> torch.Tensor:
        ...
        - self.OUT:CausalLMOutputWithPast,包含logits和损失
        :return:
        """
    ...
    def generate(self, idx, stop_id=None, max_new_tokens=256, temperature=1.0, top_k=None):
        ...
        在model.eval()模式下运行。运行效率低的采样版本，没有使用键k/v cache。
        """
        index = idx.shape[1]
